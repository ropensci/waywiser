...
We'll be working with the `boston_canopy` data from the spatialsample package, fitting a linear model to associate tree canopy coverage with mean temperature. Let's turn on sf's ability to download coordinate reference systems over the internet, just in case we don't have the CRS for that data yet:
...
We'll fit a simple linear model relating canopy coverage to daily temperatures, and then generate predictions from that model. We can use `ww_local_moran_i()` to calculate the local spatial autocorrelation of our residuals at each data point:
