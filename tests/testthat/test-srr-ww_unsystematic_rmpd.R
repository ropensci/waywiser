...
test_that("srr: expected failures for ww_unsystematic_rmpd", {
  ...
  expect_no_error(
    ww_unsystematic_rmpd(missing_df, x, y, na_rm = TRUE)
  )

  #' @srrstats {G2.14b} Users can delete NA values:
  expect_no_error(
    ww_unsystematic_rmpd(missing_df, y, x, na_rm = TRUE)
  )

  #' @srrstats {G2.14b} Users can delete NA values:
  expect_no_error(
    ww_unsystematic_rmpd_vec(missing_df$x, missing_df$y, na_rm = TRUE)
  )

  #' @srrstats {G2.14b} Users can delete NA values:
  expect_no_error(
    ww_unsystematic_rmpd_vec(missing_df$y, missing_df$x, na_rm = TRUE)
  )
  ...
  #' @srrstats {G5.8a} Zero-length data:
  expect_error_or_missing(
    ww_unsystematic_rmpd(empty_df, x, y)$.estimate,
    NaN
  )
  ...
  #' @srrstats {G5.8a} Zero-length data:
  expect_error_or_missing(
    ww_unsystematic_rmpd(empty_df, y, x)$.estimate,
    NaN
  )
  ...
  #' @srrstats {G5.8c} All-NA:
  expect_error_or_missing(
    ww_unsystematic_rmpd(all_na, x, y)$.estimate,
    NA_real_
  )
  ...
  #' @srrstats {G5.8c} All-NA:
  expect_error_or_missing(
    ww_unsystematic_rmpd(all_na, y, x)$.estimate,
    NA_real_
  )
