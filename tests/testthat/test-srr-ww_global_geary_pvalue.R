...
test_that("srr: expected failures for ww_global_geary_pvalue", {
  ...
  expect_identical(
    ww_global_geary_pvalue(worldclim_predicted, predicted, response, na_action = function(x) unlist(na.pass(x)))$.estimate,
    NA_real_
  )

  #' @srrstats {G2.14b} Users can ignore NA:
  expect_identical(
    ww_global_geary_pvalue(worldclim_predicted, response, predicted, na_action = function(x) unlist(na.pass(x)))$.estimate,
    NA_real_
  )

  #' @srrstats {G2.14b} Users can ignore NA:
  expect_identical(
    ww_global_geary_pvalue_vec(worldclim_predicted$predicted, worldclim_predicted$response, worldclim_weights, na_action = function(x) unlist(na.pass(x))),
    NA_real_
  )

  #' @srrstats {G2.14b} Users can ignore NA:
  expect_identical(
    ww_global_geary_pvalue_vec(worldclim_predicted$response, worldclim_predicted$predicted, worldclim_weights, na_action = function(x) unlist(na.pass(x))),
    NA_real_
  )
  ...
  #' @srrstats {G5.8c} All-NA:
  expect_equal(
    ww_global_geary_pvalue_vec(NA_real_, NA_real_, structure(list(neighbours = 1), class = "listw"), na_action = na.pass),
    NA_real_
  )
...
test_that("other generic srr standards", {
  ...
  expect_identical(
    sf::st_crs(ww_global_geary_pvalue(guerry_modeled, predictions, Crm_prs)$geometry),
