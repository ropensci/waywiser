...
test_that("srr: expected failures for ww_systematic_mpd", {
  ...
  expect_identical(
    ww_systematic_mpd(missing_df, y, x, na_action = function(x) unlist(na.pass(x)))$.estimate,
    NA_real_
  )

  #' @srrstats {G2.14b} Users can ignore NA:
  expect_identical(
    ww_systematic_mpd(missing_df, x, y, na_action = function(x) unlist(na.pass(x)))$.estimate,
    NA_real_
  )

  #' @srrstats {G2.14b} Users can ignore NA:
  expect_identical(
    ww_systematic_mpd_vec(missing_df$y, missing_df$x, na_action = function(x) unlist(na.pass(x))),
    NA_real_
  )

  #' @srrstats {G2.14b} Users can ignore NA:
  expect_identical(
    ww_systematic_mpd_vec(missing_df$x, missing_df$y, na_action = function(x) unlist(na.pass(x))),
    NA_real_
  )

  #' @srrstats {G2.14b} Users can delete NA values:
  expect_no_error(
    ww_systematic_mpd(missing_df, x, y, na_action = na.omit)
  )

  #' @srrstats {G2.14b} Users can delete NA values:
  expect_no_error(
    ww_systematic_mpd(missing_df, y, x, na_action = na.omit)
  )

  #' @srrstats {G2.14b} Users can delete NA values:
  expect_no_error(
    ww_systematic_mpd_vec(missing_df$x, missing_df$y, na_action = na.omit)
  )

  #' @srrstats {G2.14b} Users can delete NA values:
  expect_no_error(
    ww_systematic_mpd_vec(missing_df$y, missing_df$x, na_action = na.omit)
  )
