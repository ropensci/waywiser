...
test_that("srr: expected failures for ww_systematic_mpd", {
  ...
  expect_no_error(
    ww_systematic_mpd(missing_df, x, y, na_rm = TRUE)
  )

  #' @srrstats {G2.14b} Users can delete NA values:
  expect_no_error(
    ww_systematic_mpd(missing_df, y, x, na_rm = TRUE)
  )

  #' @srrstats {G2.14b} Users can delete NA values:
  expect_no_error(
    ww_systematic_mpd_vec(missing_df$x, missing_df$y, na_rm = TRUE)
  )

  #' @srrstats {G2.14b} Users can delete NA values:
  expect_no_error(
    ww_systematic_mpd_vec(missing_df$y, missing_df$x, na_rm = TRUE)
  )
  ...
  #' @srrstats {G5.8a} Zero-length data:
  expect_error_or_missing(
    ww_systematic_mpd(empty_df, x, y)$.estimate,
    NaN
  )
  ...
  #' @srrstats {G5.8a} Zero-length data:
  expect_error_or_missing(
    ww_systematic_mpd(empty_df, y, x)$.estimate,
    NaN
  )
  ...
  #' @srrstats {G5.8c} All-NA:
  expect_error_or_missing(
    ww_systematic_mpd(all_na, x, y)$.estimate,
    NA_real_
  )
  ...
  #' @srrstats {G5.8c} All-NA:
  expect_error_or_missing(
    ww_systematic_mpd(all_na, y, x)$.estimate,
    NA_real_
  )
